...
def main():
    ...
    if args.steps is None:
        args.steps = int(args.gen_length / 2)
    
    if args.num_eos > 0:
        steps = f"numeos{args.num_eos}_masks{args.num_masks}_steps{args.steps}_{args.remasking}"
        if args.no_eot:
            steps+= "_noEot"
    ...
    while completed < len(data.index) and args.batch_size >0:
        ...
        for sample in dataset: 
            ...
            if "llada2" in modelname.lower():
                ...
                attention_mask = sample['attention_mask'].to("cuda")
                if args.num_eos >0:
                    out = generate_fix_eos(model, 
                    input_ids, attention_mask, steps=args.steps, num_masks=args.num_masks, num_eos=args.num_eos, block_length=args.block_length, temperature=0., cfg_scale=0., remasking=args.remasking, dont_add_eot=args.no_eot)
                    output_text = tokenizer.batch_decode(out[:, input_ids.shape[1]:], skip_special_tokens=True)
                else:
                    out = generate(model, input_ids, attention_mask, steps=args.steps, gen_length=args.gen_length, block_length=args.block_length, temperature=0., cfg_scale=0., remasking=args.remasking, confidence_eos_eot_inf=args.confidence_eos_eot_inf)
                    output_text = tokenizer.batch_decode(out[:, input_ids.shape[1]:], skip_special_tokens=True)
                ...
                if args.num_eos >0:
                    ...
                    def generation_tokens_hook_func(a,tokens , b):
                        tokens[:, -args.num_eos:] = torch.full((tokens.shape[0], args.num_eos), tokenizer.eos_token_id, dtype=torch.long).to(model.device)
                        return tokens

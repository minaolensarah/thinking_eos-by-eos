...
class My_LLaDA2MoeModelLM(LLaDA2MoePreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        num_masks: int = 12,
        gen_length: int = 2048,
        dont_add_eot: bool = False,
        top_p: Optional[int] = None,
        top_k: Optional[int] = None,
        eos_early_stop: bool = False,
        minimal_topk: int = 1,
        threshold: float = 1.0, #0.95,
        eos_id: int = 156892,
        mask_id: int = 156895,
    ):
        r"""
        ...
        x[:, -num_eos:] = torch.full((1, num_eos), eos_id, dtype=torch.long).to(self.device)
        end_of_turn_id = 156900
        x[:, -(num_eos)] = torch.full((1, 1), end_of_turn_id, dtype=torch.long).to(self.device)
        if not dont_add_eot:
            x[:, -num_eos] = eos_id
        ...
        for num_block in range(1):
            current_window_end = total_length #(num_block + 1) * block_length
            cur_x = x[:, :current_window_end]

            cur_attn_mask = torch.full((1, 1, current_window_end, current_window_end), 1, dtype=torch.bfloat16).to(self.device) #block_diffusion_attention_mask[

...
def generate_fix_eos(model, prompt, attention_mask=None, steps=128, num_masks=128, num_eos=0, block_length=128, temperature=0.,
             cfg_scale=0., remasking='low_confidence',dont_add_eot=False, mask_id=126336, logits_eos_inf=False, confidence_eos_eot_inf=False,eos_id=126081, random_state=None):
    '''
    Args:
    ...
    # add the eos tokens
    if eos_id == -1:
        x[:, -num_eos:] = torch.LongTensor([random.choices(list(range(126079)), k=num_eos) for i in range(prompt.shape[0])]).to(model.device)
    else:
        x[:, -num_eos:] = torch.full((prompt.shape[0], num_eos), eos_id, dtype=torch.long).to(model.device)
